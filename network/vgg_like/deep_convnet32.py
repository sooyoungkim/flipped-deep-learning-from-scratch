...
class DeepConvNet:
    """
    네트워크 구성은 아래와 같음 no.3-2
        conv - relu - conv - relu - pool - dropout -
        conv - relu - conv - relu - pool - dropout -
        affine256 - relu - dropout -
        affine10 - softmax
    """

    def __init__(self, input_dim=(1, 28, 28),  # 입력데이터 차원 수
                 # 64 * ((28 + 2*1 - 2)/1 + 1) * = 32 * 28 * 28
                 conv_param_1={'filter_num': 32, 'filter_size': 2, 'pad': 1, 'stride': 1},
                 conv_param_2={'filter_num': 32, 'filter_size': 2, 'pad': 1, 'stride': 1},
                 conv_param_3={'filter_num': 64, 'filter_size': 2, 'pad': 1, 'stride': 1},
                 conv_param_4={'filter_num': 64, 'filter_size': 2, 'pad': 1, 'stride': 1},
                 hidden_size=256,
                 output_size=10):
        conv_params = [conv_param_1, conv_param_2, conv_param_3, conv_param_4]
        ...
        # affine 계층 가중치 초기화
        self.params['W5'] = wight_init_scales[4] * np.random.randn(self.conv_output_size, hidden_size)
        ...
        self.layers['Dropout4'] = Dropout(0.3)
        ...
        #         self.params['gamma7'] = np.ones(hidden_size)  # 1
        #         self.params['beta7'] = np.zeros(hidden_size)  # 0
