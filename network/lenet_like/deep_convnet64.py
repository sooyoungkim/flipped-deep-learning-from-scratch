...
class DeepConvNet:
    """
    네트워크 구성은 아래와 같음 no.6-4 (no.6-3에서 batchNorm 위치를 변경)
    ...
    def __init__(self, input_dim=(1, 28, 28),  # 입력데이터 차원 수
                 # 32 * ((28 + 2*1 - 3)/1 + 1) * = 32 * 28 * 28 -> 모두 곱하면 25088 -> pooling 적용하면 -> 32 * ((28 - 2)/2 + 1) *  = 32 * 14 * 14
                 # 64 * ((14 + 2*2 - 3)/1 + 1) * = 64 * 16 * 16 -> 모두 곱하면 16384 -> pooling 적용하면 -> 64 * ((16 - 2)/2 + 1) *  = 64 * 8 * 8
                 # 128 * ((8 + 2*1 - 3)/1 + 1) * = 128 * 8 * 8  -> 모두 곱하면 8192  -> pooling 적용하면 -> 128 * ((8 - 2)/2 + 1) *  = 128 * 4 * 4
        ...
        # 원본 그대로에서 시작하는 것으로 초기화. 1배 확대(gamma), 이동 0(beta)
        self.params['gamma2'] = np.ones(16384)  # 1
        self.params['beta2'] = np.zeros(16384)  # 0
        ...
        # 원본 그대로에서 시작하는 것으로 초기화. 1배 확대(gamma), 이동 0(beta)
        self.params['gamma3'] = np.ones(8192)  # 1
        self.params['beta3'] = np.zeros(8192)  # 0
