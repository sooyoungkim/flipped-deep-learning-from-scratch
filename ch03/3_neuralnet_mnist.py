...
def get_data():
    ...
    #       - 신경망의 입력데이터에 특정 변환을 가하는 것 : 전처리라 한다.
    #       - 입력이미지에 대한 전처리 작업으로 정규화를 수행한셈이다.
...
def init_network():
    ...
    #   => 입력층 뉴런수 : 784 -> 은닉층1의 뉴런수 : 50 -> 은닉층2의 뉴런수 : 100 -> 출력층 뉴런수 : 10 개 로 구성됨을 알 수 있다.
...
def predict(network, x):
    ...
    # 추론단계에서는 출력층의 소프트맥스 함수를 생략하는 것이 일반적이다.(신경망 학습시에는 사용한다.)
    # 생략
...
print("Accuracy:" + str(float(accuracy_cnt) / len(x)))  # (맞힌 예측 수 / 전체 이미지 갯수) -> 정확도 구한다. Accuracy:0.9352
